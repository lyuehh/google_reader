---
layout: post
title:  "jquery1.43源码分析之核心部分"
date:   2010-10-13 02:53:05
author: 
categories: program
---
...
## jquery1.43源码分析之核心部分
### by 
### at 2010-10-13 02:53:05
### original <http://www.javaeye.com/topic/783260>
...
<br>会陆续放上来.
<br>
<br>这些源码分析大部分都是自己一个人的理解, 所以错误和缺陷肯定是有的. 写成文档的目的一是为了梳理自己的知识,再就是希望在和大家的讨论中, 能认识和改正自己的错误.
<br>
<br>最后面提供了pdf下载.
<br>
<br>ps: pdf的排版还是不满意, 哪位知道有好的免费word转pdf软件下载? 我现在用的在线转换半小时才能转一次,不太方便.
<br>
<br>
<br>
<br>
...
<br><pre name="code">(function(window,  undefined){
…..
})(window)</pre>
...
<br><pre name="code">var jQuery = (function(){   --------------------jq1
...
<br><pre name="code">window.jQuery = window.$ = jQuery;
...
<br><pre name="code">function Man(name){
	this.name = name;	
}
...
<br>}
<br>而是jQuery.fn.init.
<br><pre name="code">var jQuery  =  function( selector, context ) {			
...
<br><pre name="code">get: function( num ) {
		return num == null ? 
...
<br>比如push
<br><pre name="code">function ArrayPush() {
...
<br><pre name="code">index: function( elem ) {
...
<br><pre name="code">if ( Array.prototype.indexOf ) {
...
<br>然后
<br>	<pre name="code">inArray: function( elem, array ) {
		if ( array.indexOf ) {
...
<br><span style="font-size:large">[/size]</span>[/size]
  ...
  <ul>
    本文附件下载:
      ...
      <li><a href="http://dl.javaeye.com/topics/download/8bae6532-4e49-3be6-a62d-367613fe5259">jquery-1.4.3rc1.zip</a> (50.3 KB)</li>
    
      <li><a href="http://dl.javaeye.com/topics/download/f3b593c5-cbfb-3190-9cd6-80f8bf3c6909">jquery1.43源码分析_核心部分_.zip</a> (280.3 KB)</li>
          ...
          <span style="color:red">
            <a href="http://www.javaeye.com/topic/783260" style="color:red">已有 <strong>4</strong> 人发表回复，猛击-&gt;&gt;<strong>这里</strong>&lt;&lt;-参与讨论</a>
          </span>
          <br><br><br>
