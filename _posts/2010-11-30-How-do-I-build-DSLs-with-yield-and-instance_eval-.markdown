...
<p>So how would we re-implement the <code>map</code> functionality if it weren’t provided for us? It’s actually quite simple using the <code>yield</code> statement:</p>
<pre>class Array
  def my_map
    result = []
    self.each do |item|
